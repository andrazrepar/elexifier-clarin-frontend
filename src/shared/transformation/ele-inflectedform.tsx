...
import { ElePronunciation } from "./ele-pronunciation";

export const EleInflectedForm: React.FC<any> = (props) => {
	...
	const [inflectedFormLabelCount, setInflectedFormLabelCount] = useState(() => {
	...
	useEffect(() => {
		...
		const newCount = countExistingEMDs(
			...
			inputs,
			inflectedFormLabelCount
		);

		setInflectedFormLabelCount(newCount);
	}, [props.count[localPath], props.flatTransformation, localPath]);

	const [inflectedFormPronunciationCount, setInflectedFormPronunciationCount] =
		useState(() => {
			// Create an array of inputs
			const inputs = Array.from(
				{ length: props.count[localPath] },
				(_, index) => `${localPath}${index}-pronunciation`
			);
	...
	useEffect(() => {
		...
		const newCount = countExistingEMDs(
			props.flatTransformation,
			inputs,
			inflectedFormPronunciationCount
		);

		setInflectedFormPronunciationCount(newCount);
	}, [props.count[localPath], props.flatTransformation, localPath]);
				...
				return (
					...
					>
						...
						<EleLabel
							...
							handleExpand={toggleChildElement}
							flatTransformation={props.flatTransformation}
							count={inflectedFormLabelCount}
							setCount={setInflectedFormLabelCount}
						/>
						<ElePronunciation
							id={`${localPathIndex}`}
							...
							handleExpand={toggleChildElement}
							flatTransformation={props.flatTransformation}
							count={inflectedFormPronunciationCount}
							setCount={setInflectedFormPronunciationCount}
						/>
