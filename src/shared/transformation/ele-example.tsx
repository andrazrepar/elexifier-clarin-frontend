...
export const EleExample: React.FC<any> = (props) => {
	...
	const [exampleLabelCount, setExampleLabelCount] = useState(() => {
			...
			{Array.from({ length: props.count[localPath] }).map((_, index) => {
				...
				return (
					...
					>
						...
						<EleInputField
							label="Sound file"
							name={`${localPathIndex}-soundFile`}
							className="text-sm font-medium text-indigo-600"
							existingValue={
								props.flatTransformation[`${localPathIndex}-soundFile`]
							}
						/>
						<EleTMD
						...
						<EleTMD
							id={`${localPathIndex}-sourceIdentity`}
						...
						<EleTMD
							id={`${localPathIndex}-sourceElaboration`}
						...
						<EleLabel
							...
							handleExpand={toggleChildElement}
							flatTransformation={props.flatTransformation}
							count={exampleLabelCount}
							setCount={setExampleLabelCount}
						/>
