...
export function TransformTree({
	...
	path = "", // Add a new "path" prop, default to an empty string
}) {
	...
	const currentPath =
		path === ""
			? transformTreeStructure.outElement
			: `${path}-${transformTreeStructure.outElement}`;
	...
	return (
		<div className="my-2 bg-white ">
			...
			/>
			<div className={`pl-4 mt-2 ${isExpanded ? "block" : "hidden"}`}>
				{transformTreeStructure.textVals?.map((textVal, index) => (
					<div key={index} className="mt-3">
						...
						<div
							className={`pl-4 mt-2 ${
								isSubElementExpanded[index] ? "block" : "hidden"
							}`}
						>
							{Object.keys(textVal)
								.filter(
								...
								.map((key) => (
									<EleInputField
										key={key}
										name={`${currentPath}-textVals-${textVal.outElement}-${key}`}
										label={key}
									/>
								))}
							<EleCollapsibleElement
							...
							<div
								className={`pl-4 mt-2 ${
									isAdvancedExpanded[index] ? "block" : "hidden"
								}`}
							>
